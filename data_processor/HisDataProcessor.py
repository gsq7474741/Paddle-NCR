...
class HisDataProcessor(DataProcessor):
    data_columns = ['X', cfg.C_HISTORY, cfg.C_HISTORY_LENGTH]
    ...
    def _get_feed_dict_rt(self, data, batch_start, batch_size, train):
        ...
        for c in self.data_columns:
            ...
            if c == cfg.C_HISTORY and self.sparse_his == 1:
                x, y = [], []
    ...
    def format_data_dict(self, df):
        if cfg.C_HISTORY in df:
            history = df[[cfg.C_HISTORY]]
        else:
            uids = df[['uid']]
            history = pd.merge(uids, self.data_loader.train_his_df, on=\
                'uid', how='left')
            history = history.rename(columns={'iids': cfg.C_HISTORY})
        history[cfg.C_HISTORY] = history[cfg.C_HISTORY].fillna('')
        data_dict = DataProcessor.format_data_dict(self, df)
        if self.max_his > 0 and self.sup_his == 1:
            data_dict[cfg.C_HISTORY] = history[cfg.C_HISTORY].apply(
