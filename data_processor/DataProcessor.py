...
class DataProcessor(object):
    ...
    def get_train_data(self, epoch):
        if self.train_data is None or epoch < 0:
            ...
            self.train_data[cfg.K_SAMPLE_ID] = np.arange(0, len(self.
                                                                train_data['Y']))
        if epoch >= 0:
    ...
    def get_validation_data(self):
        if self.validation_data is None:
            ...
            self.validation_data[cfg.K_SAMPLE_ID] = np.arange(0, len(
    ...
    def get_test_data(self):
        if self.test_data is None:
            ...
            self.test_data[cfg.K_SAMPLE_ID] = np.arange(0, len(self.
                                                               test_data['Y']))
    ...
    def _get_feed_dict_rt(self, data, batch_start, batch_size, train):
        ...
        feed_dict = {'train': train, 'rank': 0, cfg.K_SAMPLE_ID: data[
            cfg.K_SAMPLE_ID][batch_start:batch_start + real_batch_size]}
        if 'Y' in data:
    ...
    def _get_feed_dict_rk(self, data, batch_start, batch_size, train,
        ...
        if not train:
            ...
            sample_id = data[cfg.K_SAMPLE_ID][batch_start:batch_start +
                                                          real_batch_size]
            neg_sample_id = sample_id + len(self.train_data['Y'])
            feed_dict = {'train': train, 'rank': 1, 'Y': utils.
                numpy_to_torch(y), cfg.K_SAMPLE_ID: np.concatenate([
