...
class NCR(BaseModel):
    ...
    def predict(self, feed_dict):
        ...
        uh_vectors = paddle.reshape(user_vectors,[user_vectors.size(0), 1,user_vectors.size(1)])
        ...
        for i in shuffled_history_idx:
            tmp_vector = self.logic_or(tmp_vector, self.logic_not(
                his_vectors[:, i]))
        ...
        if feed_dict['rank'] == 1:
            ...
            prediction = paddle.nn.functional.cosine_similarity(sent_vector,paddle.reshape(self.true,[1, -1])) * (self.label_max - self.label_min
    ...
    def forward(self, feed_dict):
        ...
        r_not_not_true = paddle.sum((1 - paddle.nn.functional.cosine_similarity(self.logic_not(self.logic_not(self.true)), self.true, axis=0)))
        r_not_not_self = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_not(self.logic_not(constraint)), constraint, axis=dim))
        r_not_self = paddle.mean(1 + paddle.nn.functional.cosine_similarity(self.logic_not(constraint), constraint, axis=dim))
        r_not_self = paddle.mean(1 + paddle.nn.functional.cosine_similarity(self.logic_not(constraint), constraint, axis=dim))
        r_not_not_not = paddle.mean(1 + paddle.nn.functional.cosine_similarity(self.logic_not(self.logic_not(constraint)), self.logic_not(constraint), axis=dim))
        r_and_true = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_and(constraint, self.true.expand_as(constraint)),constraint, axis=dim))
        r_and_false = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_and(constraint, false.expand_as(constraint)), false.expand_as(constraint), axis=dim))
        r_and_self = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_and(constraint, constraint), constraint, axis=dim))
        r_and_not_self = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_and(constraint, self.logic_not(constraint)), false.expand_as(constraint), axis=dim))
        r_and_not_self_inverse = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_and(self.logic_not(constraint),constraint), false.expand_as(constraint), axis=dim))
        r_or_true = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_or(constraint, self.true.expand_as(constraint)), self.true.expand_as(constraint), axis=dim))
        r_or_false = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_or(constraint, false.expand_as(constraint)), constraint,axis=dim))
        r_or_self = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_or(constraint, constraint), constraint, axis=dim))
        r_or_not_self = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_or(constraint, self.logic_not(constraint)), self.true.expand_as(constraint), axis=dim))
        r_or_not_self_inverse = paddle.mean(1 - paddle.nn.functional.cosine_similarity(self.logic_or(self.logic_not(constraint), constraint), self.true.expand_as(constraint), axis=dim))
