...
class DataLoader(object):
    ...
    def __init__(self, path, dataset, label='label', load_data=True, sep=\
        ...
        self.train_file = os.path.join(self.path, dataset + cfg.
                                       TRAIN_SUFFIX)
        self.validation_file = os.path.join(self.path, dataset + cfg.
                                            VALIDATION_SUFFIX)
        self.test_file = os.path.join(self.path, dataset + cfg.TEST_SUFFIX
                                      )
        self.info_file = os.path.join(self.path, dataset + cfg.INFO_SUFFIX
                                      )
        self.user_file = os.path.join(self.path, dataset + cfg.USER_SUFFIX
                                      )
        self.item_file = os.path.join(self.path, dataset + cfg.ITEM_SUFFIX
                                      )
        self.train_his_file = os.path.join(self.path, dataset + cfg.
                                           TRAIN_GROUP_SUFFIX)
        self.vt_his_file = os.path.join(self.path, dataset + cfg.
                                        VT_GROUP_SUFFIX)
        ...
        for df in [self.train_df, self.validation_df, self.test_df]:
            ...
            for i, uid in enumerate(uids):
                ...
                if label <= 0 and not neg_column and neg:
                    ...
                    his_dict[uid].append(iid)
            df[cfg.C_HISTORY] = history
            if neg and neg_column:
                df[cfg.C_HISTORY_NEG] = neg_history
